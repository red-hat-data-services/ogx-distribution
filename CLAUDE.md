...
1. **`build/build.py`** (hook: `pkg-gen`, always runs) — creates a temp venv, installs OGX, runs `ogx stack list-deps` against `build/build.yaml` to discover Python dependencies, then generates:
   ...
   - `distribution/requirements-lock.txt` — midstream lock file (pinned versions with hashes, resolved against PyPI)
   - `distribution/requirements-lock-konflux.txt` — downstream lock file (only when `RHAI_INDEX_URL` is set in `build/build.env`; pinned versions with hashes, resolved against the RHAI internal index)
...
- `distribution/requirements-lock.txt` — midstream lock file, generated by `build/build.py`
- `distribution/requirements-lock-konflux.txt` — downstream lock file, generated by `build/build.py` when `RHAI_INDEX_URL` is set
...
- **`build/build.env`** — sets `OGX_VERSION`, `OGX_INSTALL_FROM_SOURCE`, and `RHAI_INDEX_URL`. When `RHAI_INDEX_URL` is set to the internal RHAI package index URL, `build.py` generates a downstream lock file (`requirements-lock-konflux.txt`) in addition to the midstream one.
