...
To learn more about the distribution image content, see the [README](dist/README.md) in the `dist/` directory.
...
The `dist/config.yaml` and `dist/requirements.txt` files are auto-generated by `build/build.py`. To regenerate them:
...
- Execute `./build/build.py` to generate `dist/config.yaml` and `dist/requirements.txt`
- Execute `./build/gen_distro_docs.py` to regenerate `dist/README.md`

> [!WARNING]
> Do not edit `dist/config.yaml`, `dist/requirements.txt`, or `dist/README.md` manually — they are regenerated on every commit.
...
> [!TIP]
> Ensure you include any env vars you might need for providers in the container env - you can read more about that [here](dist/README.md).
