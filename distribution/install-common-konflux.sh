...
HF_CACHE="${APP_ROOT}/.cache/huggingface/hub"
...
for f in .gitattributes config.json model.onnx model.safetensors modules.json \
         pytorch_model.bin README.md sentence_bert_config.json \
         special_tokens_map.json tokenizer.json tokenizer_config.json \
         trainer_state.json vocab.json; do
...
mkdir -p "${HERON_DIR}"
for f in .gitattributes config.json docling_heron_400.png model.safetensors \
         preprocessor_config.json README.md; do
...
mkdir -p "${TF_DIR}/model_artifacts/tableformer/accurate" \
         "${TF_DIR}/model_artifacts/tableformer/fast"
for f in .gitattributes .gitignore config.json README.md; do
    cp "${CACHI2_GENERIC}/docling-models/${f}" "${TF_DIR}/${f}"
done
...
cp "${CACHI2_GENERIC}/docling-models/model_artifacts/tableformer/fast/tableformer_fast.safetensors" \
   "${TF_DIR}/model_artifacts/tableformer/fast/tableformer_fast.safetensors"
cp "${CACHI2_GENERIC}/docling-models/model_artifacts/tableformer/fast/tm_config.json" \
   "${TF_DIR}/model_artifacts/tableformer/fast/tm_config.json"

# ---------- DocumentFigureClassifier-v2.5 ----------
FIGCLS_DIR="${DOCLING_ARTIFACTS_PATH}/docling-project--DocumentFigureClassifier-v2.5"
mkdir -p "${FIGCLS_DIR}"
for f in .gitattributes ConfusionMatrix_v2_0.png ConfusionMatrix_v2_5.png config.json \
         model.onnx model.safetensors preprocessor_config.json README.md; do
...
mkdir -p "${RAPIDOCR_DIR}/onnx/PP-OCRv4/cls" \
         ...
         "${RAPIDOCR_DIR}/torch/PP-OCRv4/cls" \
         "${RAPIDOCR_DIR}/torch/PP-OCRv4/det" \
         "${RAPIDOCR_DIR}/torch/PP-OCRv4/rec"
...
cp "${CACHI2_GENERIC}/rapidocr/torch/PP-OCRv4/cls/ch_ptocr_mobile_v2.0_cls_mobile.pth" \
   "${RAPIDOCR_DIR}/torch/PP-OCRv4/cls/ch_ptocr_mobile_v2.0_cls_mobile.pth"
cp "${CACHI2_GENERIC}/rapidocr/torch/PP-OCRv4/det/ch_PP-OCRv4_det_mobile.pth" \
   "${RAPIDOCR_DIR}/torch/PP-OCRv4/det/ch_PP-OCRv4_det_mobile.pth"
cp "${CACHI2_GENERIC}/rapidocr/torch/PP-OCRv4/det/en_PP-OCRv3_det_mobile.pth" \
   "${RAPIDOCR_DIR}/torch/PP-OCRv4/det/en_PP-OCRv3_det_mobile.pth"
cp "${CACHI2_GENERIC}/rapidocr/torch/PP-OCRv4/rec/ch_PP-OCRv4_rec_mobile.pth" \
   "${RAPIDOCR_DIR}/torch/PP-OCRv4/rec/ch_PP-OCRv4_rec_mobile.pth"
cp "${CACHI2_GENERIC}/rapidocr/torch/PP-OCRv4/rec/en_PP-OCRv4_rec_mobile.pth" \
   "${RAPIDOCR_DIR}/torch/PP-OCRv4/rec/en_PP-OCRv4_rec_mobile.pth"
